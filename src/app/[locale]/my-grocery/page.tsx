...
export default function MyGroceryPage() {
  ...
  const [listToDelete, setListToDelete] = useState<GroceryEvent | null>(null)
  ...
  const handleDeleteClick = (list: GroceryEvent) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true)
    try {
      const response = await fetch(`/api/grocery/${listToDelete.share_token}`, {
        method: 'DELETE'
      })
      const data = await response.json()
        ...
        <AlertDialogContent dir="rtl" className="max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-slate-600 text-right">
              {t('deleteConfirmDescription', { eventName: listToDelete?.event_name || '' })}
            </AlertDialogDescription>
          </AlertDialogHeader>
