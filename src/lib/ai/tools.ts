...
- אחרת השתמש בברירות מחדל: הישגים, ספורט, פרסים, אירועים, הודעות`,
          ...
          event_date: {
            type: 'string',
            description: 'תאריך שבו קרה ההישג/אירוע (YYYY-MM-DD). נסה לחלץ מההודעה ("20.1.2026", "אתמול", "שבוע שעבר"). אם לא נמצא - שאל!',
          },
          start_date: {
            type: 'string',
            description: 'תאריך התחלת הצגה בקרוסלה (YYYY-MM-DD). **תמיד השאר null/undefined** - הדגשות מוצגות מיד מרגע היצירה.',
          },
          end_date: {
            type: 'string',
            description: `תאריך סיום הצגה בקרוסלה (YYYY-MM-DD). **חובה לשאול את המשתמש!**

שאל: "עד מתי להציג?" עם אופציות:
1=עד יום אחרי האירוע (event_date + 1 day - רק אם יש event_date!)
2=שבוע (+7 ימים מהיום)
3=שבועיים (+14 ימים מהיום)
4=חודש (+30 ימים מהיום, מומלץ)
5=חודשיים (+60 ימים מהיום)
6=עד סוף שנה (30.6.2026)

אתה מחשב:
- אם בחר 1 והיש event_date → end_date = event_date + 1 day
- אם בחר 2-5 → end_date = CURRENT_DATE + [ימים]
- דוגמה: היום 3.1.2026, event_date 20.1.2026, בחר "1" → end_date = "2026-01-21"
- דוגמה: היום 3.1.2026, בחר "4" → end_date = "2026-02-02"`,
          },
          cta_text_he: {
            type: 'string',
